...
const API = axios.create({
  baseURL: 'http://192.168.0.125:8000', // Laravel base URL
  headers: {
...
const postData = async (payload,location,cb) => {
  const response = await API.post(location, payload);
  cb(response.data)
};


const getData = async (payload,location) => {
  const response = await API.get(location, payload);
  cb(response.data)
};
...
let users = []
 ...
 const login =  (email, password, role,cb) => {
    var userData = {}
    userData.email = email
    userData.password =  password
    userData.role = role
    console.log(userData)
    postData(userData,'/login',function(data){
  ...
  function register(userData,cb) {
    postData(userData,'/register',function(data){
      if(data.user === null){
      currentUser =  data.user
      cb(data)
      }
  ...
  function renderLoginPage() {
    ...
    document.getElementById('login-form').addEventListener('submit', function(e) {
      ...
      login(email, password, role,function(result){
        if (result.success) {
  ...
  function renderRegisterPage() {
    ...
    document.getElementById('register-form').addEventListener('submit', function(e) {
      ...
      register(userData,function(result){
        if (result.success) {
          navigateTo('/dashboard');
        } else {
  ...
  function renderRoomsPage() {
    postData({},"/getRooms",function(res){
      rooms = res
    })
    const content = `
      <div class="container">
